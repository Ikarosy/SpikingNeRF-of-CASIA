...
data = dict(
    ...
    #SNN-related miscs
    spiking_mode=False,           #spiking mode
)
...
coarse_model_and_render = dict(
    ...
    ### snn related
    unsqueeze_masking=False,      # if true, then masked samples are reserved as 0.
    ray_tempo_unmatch=False,      # if true, then ray has nothing to do with temporal dimension of SNN.
    condensing=False,      # if true, then ray has nothing to do with temporal dimension of SNN.
    poisson=False,      # if true, then ray has nothing to do with temporal dimension of SNN.
    tempo_flip=False,             # if true, the temporal axis will flip
    time_step=1,                  # only valid if ray_tempo_unmatch is True
    lif_type='lif',               # for those accommodate cupy
    single_step_snn=False,               # most naive way
)
