...
from lib import utils, dvgo, dcvgo, dmpigo, spiking_dvgo
from lib.load_data import load_data

from torch_efficient_distloss import flatten_eff_distloss

is_data_parall = torch.cuda.is_available() and (torch.cuda.device_count() > 1)
def config_parser():
    ...
    # snn related
    parser.add_argument("--op_count", action='store_true')
    parser.add_argument("--use_ann_trained_fine", action='store_true')
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
                      ...
                      eval_ssim=False, eval_lpips_alex=False, eval_lpips_vgg=False, op_count=False):
    '''Render images for the given viewpoints; run evaluation if gt given.
        Only the fine model is used in this function.
    '''
    assert len(render_poses) == len(HW) and len(HW) == len(Ks)

    if render_factor!=0:    # render_factor is 0 by default, then for i, c2w in enumerate(tqdm(render_poses)): one pic on loop
        HW = np.copy(HW)
        Ks = np.copy(Ks)
    ...
    times = []
    if op_count:
        print('activate the firing counter to get the average firing rate per neuron per pic')
        if is_data_parall:
            model.module.activate_op_counter()
        else:
            model.activate_op_counter()
        flop_count = []
        add_count = []
        if cfg.data.spiking_mode:
            input_sparsity_count = []
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if cfg.data.spiking_mode:
            op_keys = ['add_count', 'flop_count']
            keys_ex = ['rgb_marched', 'depth', 'alphainv_last', 'add_count', 'flop_count', 'sparsity']
        else:
            op_keys = ['add_count', 'flop_count']
            keys_ex = ['rgb_marched', 'depth', 'alphainv_last', 'add_count', 'flop_count']
        ...
        if op_count:
            render_result_chunks = [
                {k: v for k, v in model(ro, rd, vd, **render_kwargs).items() if k in keys_ex}
                for ro, rd, vd in zip(rays_o.split(8192, 0), rays_d.split(8192, 0), viewdirs.split(8192, 0))
            ]
            op_result = {
                k: torch.cat([ret[k] for ret in render_result_chunks]).sum()
                for k in op_keys
            }
            if cfg.data.spiking_mode: #sparsity
                op_result.update({
                    "sparsity": torch.cat([ret['sparsity'] for ret in render_result_chunks]).mean()
                                  })
        ...
        time_gap = end_rep - start_rep
        rgb = render_result['rgb_marched'].cpu().numpy()
        depth = render_result['depth'].cpu().numpy()
        bgmap = render_result['alphainv_last'].cpu().numpy()
        if op_count:
            flop = op_result['flop_count'].cpu().numpy()
            add = op_result['add_count'].cpu().numpy()
            flop_count.append(flop)
            add_count.append(add)
            if cfg.data.spiking_mode:
                sparsity = op_result['sparsity'].cpu().numpy()
                input_sparsity_count.append(sparsity)
        ...
        if gt_imgs is not None and render_factor==0:    #gt_imgs is the groundtruth pic
    ...
    print('Time cost for rendering', np.mean(times), '(avg)')
    if len(psnrs):
    ...
    if op_count:
        print('Counting float ops', np.mean(flop_count), '(avg)')
        print('Counting s-ops', np.mean(add_count), '(avg)')
        if cfg.data.spiking_mode:
            print('Counting input sparsity', np.mean(input_sparsity_count), '(avg)')
    ...
    if op_count:
        if is_data_parall:
            model.module.deactivate_op_counter()
        else:
            model.deactivate_op_counter()
...
def create_new_model(cfg, cfg_model, cfg_train, xyz_min, xyz_max, stage, coarse_ckpt_path, ann_pretrain, snn_ckpt_path):
    model_kwargs = copy.deepcopy(cfg_model)
    num_voxels = model_kwargs.pop('num_voxels')
    if snn_ckpt_path is not None and ann_pretrain:
        ann_fine_path = os.path.join(snn_ckpt_path.split('_trial_on')[0], 'fine_last.tar')
        ckpt = torch.load(ann_fine_path)
        num_voxels = ckpt['model_kwargs']['num_voxels']
        for k, v in model_kwargs.items():
            if k in ckpt['model_kwargs']:
                model_kwargs[k] = ckpt['model_kwargs'][k]
        coarse_ckpt_path = os.path.join(snn_ckpt_path.split('_trial_on')[0], 'coarse_last.tar')

    if len(cfg_train.pg_scale) and not ann_pretrain:
    ...
    if cfg.data.ndc:
        ...
        model = dcvgo.DirectContractedVoxGO(
            ...
            num_voxels=num_voxels,
            **model_kwargs)
    elif cfg.data.spiking_mode:
        print(f'scene_rep_reconstruction ({stage}): \033[96muse dense voxel grid\033[0m')
        model = spiking_dvgo.DirectVoxGO(
            xyz_min=xyz_min, xyz_max=xyz_max,
            num_voxels=num_voxels,
    ...
    if is_data_parall:
        ...
        optimizer = utils.create_optimizer_or_freeze_model(model.module, cfg_train, global_step=0)
    else:
...
def load_existed_model(args, cfg, cfg_train, reload_ckpt_path):
    if cfg.data.ndc:
        ...
        model_class = spiking_dvgo.DirectVoxGO
    else:
        model_class = dvgo.DirectVoxGO
    if is_data_parall:
        model = torch.nn.DataParallel(utils.load_model(model_class, reload_ckpt_path)).to(device)
        optimizer = utils.create_optimizer_or_freeze_model(model.module, cfg_train, global_step=0)
    else:
        model = utils.load_model(model_class, reload_ckpt_path).to(device)
        optimizer = utils.create_optimizer_or_freeze_model(model, cfg_train, global_step=0)
    model, optimizer, start = utils.load_checkpoint(
            model, optimizer, reload_ckpt_path, args.no_reload_optimizer, multi_gpu=(is_data_parall))
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    if reload_ckpt_path is None:
        ...
        model, optimizer = create_new_model(cfg, cfg_model, cfg_train, xyz_min, xyz_max, stage, coarse_ckpt_path, ann_pretrain=(args.use_ann_trained_fine and stage == 'fine'), snn_ckpt_path=None)
        start = 0
        if cfg_model.maskout_near_cam_vox:
            if is_data_parall:
                model.module.maskout_near_cam_vox(poses[i_train,:3,3], near)
            else:
    ...
    if args.use_ann_trained_fine and stage == 'fine':
        model = utils.load_ann_params(model, last_ckpt_path, is_data_parall)
        # freeze the voxel representation
        for name, param in model.named_parameters():
            if 'rgbnet' not in name:
    ...
    def gather_training_rays():
        ...
        if cfg_train.ray_sampler == 'in_maskcache':
            rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = dvgo.get_training_rays_in_maskcache_sampling(
                    ...
                    ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
                    flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y,
                    model=model.module if is_data_parall else model, render_kwargs=render_kwargs)
    ...
    if cfg_train.pervoxel_lr:
        def per_voxel_init():
            if is_data_parall:
                cnt = model.module.voxel_count_views(
                        rays_o_tr=rays_o_tr, rays_d_tr=rays_d_tr, imsz=imsz, near=near, far=far,
                        stepsize=cfg_model.stepsize, downrate=cfg_train.pervoxel_lr_downrate,
                        irregular_shape=data_dict['irregular_shape'])
                optimizer.set_pervoxel_lr(cnt)
                if model.module.mask_cache is not None:
                    model.module.mask_cache.mask[cnt.squeeze() <= 2] = False
            else:
    ...
    if cfg_train.maskout_lt_nviews > 0:
        if is_data_parall:
            model.module.update_occupancy_cache_lt_nviews(
    ...
    time0 = time.time()
    global_step = -1
    pretrain_rec = 0 if not args.use_ann_trained_fine else 30000
    for global_step in trange(1+start + pretrain_rec, 1+cfg_train.N_iters + pretrain_rec):

        # renew occupancy grid
        if is_data_parall:
            if model.module.mask_cache is not None and (global_step + 500) % 1000 == 0:
                model.module.update_occupancy_cache()
        else:
        ...
        if is_data_parall:
            if global_step in cfg_train.pg_scale:
                ...
                if isinstance(model.module, (dvgo.DirectVoxGO, dcvgo.DirectContractedVoxGO, spiking_dvgo.DirectVoxGO)):
                    model.module.scale_volume_grid(cur_voxels)
                elif isinstance(model.module, dmpigo.DirectMPIGO):
                    model.module.scale_volume_grid(cur_voxels, model.module.mpi_depth)
                else:
                    raise NotImplementedError
                optimizer = utils.create_optimizer_or_freeze_model(model.module, cfg_train, global_step=0)
                model.module.act_shift -= cfg_train.decay_after_scale
                torch.cuda.empty_cache()
        else:
            if global_step in cfg_train.pg_scale:
                n_rest_scales = len(cfg_train.pg_scale)-cfg_train.pg_scale.index(global_step)-1
                cur_voxels = int(cfg_model.num_voxels / (2**n_rest_scales))
                if isinstance(model, (dvgo.DirectVoxGO, dcvgo.DirectContractedVoxGO, spiking_dvgo.DirectVoxGO)):
        ...
        if cfg_train.weight_nearclip > 0:
            near_thres = data_dict['near_clip'] / model.scene_radius[0].item() if not is_data_parall else data_dict['near_clip'] / model.module.scene_radius[0].item()
        ...
        if (global_step - pretrain_rec)<cfg_train.tv_before and (global_step - pretrain_rec)>cfg_train.tv_after and (global_step - pretrain_rec)%cfg_train.tv_every==0:
            if cfg_train.weight_tv_density>0:
                if is_data_parall:
                    model.module.density_total_variation_add_grad(
                    cfg_train.weight_tv_density/len(rays_o), (global_step - pretrain_rec)<cfg_train.tv_dense_before)
                else:
                    model.density_total_variation_add_grad(
                    cfg_train.weight_tv_density/len(rays_o), (global_step - pretrain_rec)<cfg_train.tv_dense_before)
            if cfg_train.weight_tv_k0>0:
                if is_data_parall:
                    model.module.k0_total_variation_add_grad(
                    cfg_train.weight_tv_k0/len(rays_o), (global_step - pretrain_rec)<cfg_train.tv_dense_before)
                else:
        ...
        if global_step%args.i_weights==0:
            ...
            torch.save({
                ...
                'model_kwargs': model.module.get_kwargs() if is_data_parall else model.get_kwargs(),
                'model_state_dict': model.module.state_dict() if is_data_parall else model.module.state_dict(),
    ...
    if global_step != -1:
        torch.save({
            'global_step': global_step,
            'model_kwargs': model.module.get_kwargs() if is_data_parall else model.get_kwargs(),
            'model_state_dict': model.module.state_dict() if is_data_parall else model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
        }, last_ckpt_path)
...
def train(args, cfg, data_dict):
    ...
    if cfg.coarse_train.N_iters == 0:
        ...
        xyz_min_fine, xyz_max_fine = compute_bbox_by_coarse_geo(
                model_class=spiking_dvgo.DirectVoxGO, model_path=coarse_ckpt_path,
...
if __name__=='__main__':
    ...
    if args.export_coarse_only:
        ...
        with torch.no_grad():
            ...
            if is_data_parall:
                model = torch.nn.DataParallel(utils.load_model(dvgo.DirectVoxGO, ckpt_path)).to(device)
                alpha = model.module.activate_density(model.module.density.get_dense_grid()).squeeze().cpu().numpy()
                rgb = torch.sigmoid(model.module.k0.get_dense_grid()).squeeze().permute(1,2,3,0).cpu().numpy()
            else:
    ...
    if args.render_test or args.render_train or args.render_video:
        ...
        if cfg.data.ndc:
            ...
            model_class = dvgo.DirectVoxGO
        if is_data_parall:
            model = torch.nn.DataParallel(utils.load_model(model_class, ckpt_path)).to(device)
        else:
        ...
        render_viewpoints_kwargs = {
            ...
            'op_count': (args.op_count and (args.render_video_factor == 0))
        }
