...
''' Checkpoint utils
'''
def load_checkpoint(model, optimizer, ckpt_path, no_reload_optimizer, multi_gpu=False):
...
def load_ann_params(model, snn_ckpt_path, multi_gpu):
    ann_fine_path = os.path.join(snn_ckpt_path.split('_trial_on')[0], 'fine_last.tar')
    ckpt = torch.load(ann_fine_path)
    new_ckpt = {}
    for k, v in ckpt.items():
        if not k == 'model_state_dict':
            new_ckpt[k] = v
        else:
            new_ckpt[k] = {}
            for k1, v1 in ckpt[k].items():
                if not 'rgbnet' in k1:
                    new_ckpt[k][k1] = v1
                elif 'weight' in k1:
                    for idx,i in enumerate(k1.split('.')):
                        if i.isdigit():
                            print(i)
                            if i == '0':
                                new_ckpt[k]['rgbnet.layer1.0.weight'] = v1
                            elif i == '2':
                                new_ckpt[k]['rgbnet.layer2.0.0.weight'] = v1
                            elif i == '3':
                                new_ckpt[k]['rgbnet.layer3.weight'] = v1
                            break
    if multi_gpu:
        model.module.load_state_dict(new_ckpt['model_state_dict'])
    else:
