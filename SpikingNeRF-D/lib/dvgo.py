...
class DirectVoxGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        if rgbnet_dim <= 0:
            ...
            dim0 = (3+3*viewbase_pe*2) #view_embed as base dims
            if self.rgbnet_full_implicit:
                dim0 += 3
            elif rgbnet_direct:
                dim0 += self.k0_dim
    ...
    def _set_grid_resolution(self, num_voxels):
        ...
        self.world_size = ((self.xyz_max - self.xyz_min) / self.voxel_size).long()  #bbox 的边长 有多少个 voxel
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        # print("debuging "*10)
        # print('N', N)
        # print('ray_pts', ray_pts[:10])
        # print('ray_id', ray_id[:10])
        # print('step_id', step_id[:10])
        # print("debuging "*10)
        ...
        if self.rgbnet is None:
            ...
            if self.rgbnet_direct and not self.rgbnet_full_implicit:
                k0_view = k0
            elif not self.rgbnet_full_implicit:
                k0_view = k0[:, 3:]
                k0_diffuse = k0[:, :3]
            else:
                k0_view = ray_pts
            ...
            add_count = torch.zeros(1, device=rgb_feat.device)
            flop_count = torch.zeros(1, device=rgb_feat.device)
            if self.use_flop_counter:
                flop_count += rgb_feat.detach().shape[0] * (self.dim0 * self.rgbnet_width +
                                                            self.rgbnet_width * self.rgbnet_width * (self.rgbnet_depth-2) +
                                                            self.rgbnet_width * 3)
                if rgb_feat.numel() == 0:
