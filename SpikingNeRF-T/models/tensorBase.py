...
class MLPRender_Fea(torch.nn.Module):
    def __init__(self,inChanel, viewpe=6, feape=6, featureC=128):
        ...
        layer1 = torch.nn.Linear(self.in_mlpC, featureC, bias=False)
        layer2 = torch.nn.Linear(featureC, featureC, bias=False)
        layer3 = torch.nn.Linear(featureC,3, bias=False)
...
class TensorBase(torch.nn.Module):
    ...
    def init_render_func(self, shadingMode, pos_pe, view_pe, fea_pe, featureC, device):
        ...
        elif shadingMode == 'MLP_Fea_Spiking':
            self.renderModule = MLPRender_Fea_Spiking(self.app_dim, view_pe, fea_pe, featureC).to(device)
    ...
    def sample_ray_ndc(self, rays_o, rays_d, is_train=True, N_samples=-1):
        N_samples = N_samples if N_samples > 0 else self.nSamples # N_samples is the number of samples per ray
    ...
    def forward(self, rays_chunk, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1):
        ...
        #viewdirs[ray, pts, xyz]
        # xyz_sampled[ray, pts, xyz]
        # z_vals[ray, pts]
        # ray_valid[ray, pts, xyz]
        ...
        sigma = torch.zeros(xyz_sampled.shape[:-1], device=xyz_sampled.device)  # [ray,pts]
        rgb = torch.zeros((*xyz_sampled.shape[:2], 3), device=xyz_sampled.device) # [ray,pts,rgb]
        ...
        # op counter
        flop_count = torch.zeros(1, device=xyz_sampled.device)
        add_count = torch.zeros(1, device=xyz_sampled.device)

        if app_mask.any():
            app_features = self.compute_appfeature(xyz_sampled[app_mask])   #[ray,pts,xyz] -> [valid_samples, xyz]
            if self.shadingMode == 'MLP_Fea':
                flop_count += app_features.detach().shape[0] * (self.renderModule.in_mlpC * self.renderModule.featureC + self.renderModule.featureC * self.renderModule.featureC + self.renderModule.featureC * 3)
                valid_rgbs = self.renderModule(xyz_sampled[app_mask], viewdirs[app_mask], app_features)
            elif self.shadingMode == 'MLP_Fea_Spiking':
                flop_count += app_features.detach().shape[0] * (self.renderModule.in_mlpC * self.renderModule.featureC)
                functional.reset_net(self.renderModule)
                valid_rgbs, add = self.renderModule(xyz_sampled[app_mask], viewdirs[app_mask], app_features, app_mask)
                functional.reset_net(self.renderModule)
                add_count += add
            else: #to-do: complete the all application to Tensorf
        ...
        return rgb_map, depth_map, flop_count, add_count # rgb, sigma, alpha, weight, bg_weight
